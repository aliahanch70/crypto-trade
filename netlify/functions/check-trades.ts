...
const {
  VITE_SUPABASE_URL,
  VITE_SUPABASE_ANON_KEY, // Or SUPABASE_SERVICE_KEY if you are using it
  TELEGRAM_BOT_TOKEN,
...
function analyzeTrades(trades: Trade[], prices: Map<string, number>) {
  ...
  for (const trade of trades) {
    ...
    reportEntry += `   - Entry: \`${trade.entry_price.toFixed(4)}\`\n`;
...
export const handler = async () => {
  ...
  try {
    ...
    if (openTrades.length === 0) {
      ...
      // Optionally send a "no open trades" message to admin
      if (TELEGRAM_ADMIN_CHAT_ID) {
        await sendTelegramNotification(TELEGRAM_ADMIN_CHAT_ID, `ℹ️ Bot check complete. No open trades found.`);
      }
    ...
    // Step 2: Analyze data to get alerts and reports
